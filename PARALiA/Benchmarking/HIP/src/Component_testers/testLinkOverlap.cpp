...
int main(const int argc, const char *argv[]) {
  ...
  long int ldsrc, ldest = ldsrc = TileDim + 1;
	...
	for(short dev_id_idx = 0 ; dev_id_idx < LOC_NUM; dev_id_idx++){
  	...
  	unit_buffs[3*dev_id_idx] = CoCoMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx));
		unit_buffs[3*dev_id_idx+1] = CoCoMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx));
		unit_buffs[3*dev_id_idx+2] = CoCoMalloc(TileDim*(TileDim+1)*elemSize, deidxize(dev_id_idx));
		timer = csecond() - timer;
