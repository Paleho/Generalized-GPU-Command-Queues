...
int main(const int argc, const char *argv[]) {
    ...
    // Copy A and B in C_loc
    double *A_in_C_loc = (double*) CoCoMalloc(M * K*sizeof(double), C_loc);
    CoCoMemcpy(A_in_C_loc, A,  M * K *sizeof(double), C_loc, A_loc);
    double *B_in_C_loc = (double*) CoCoMalloc(M * K*sizeof(double), C_loc);
    CoCoMemcpy(B_in_C_loc, B,  M * K *sizeof(double), C_loc, B_loc);
	
    hipblasHandle_t blas_handle;
    assert(HIPBLAS_STATUS_SUCCESS == hipblasCreate(&blas_handle));
    hipblasOperation_t op_A = OpCharToCublas(TransA), op_B = OpCharToCublas(TransB);
    assert(HIPBLAS_STATUS_SUCCESS == hipblasDgemm(blas_handle, op_A, op_B, M, N, K, &alpha, A_in_C_loc, ldA, B_in_C_loc, ldB, &beta, C, ldC));
    ...
    CoCoMemcpy(C_out1, C,  M * N *sizeof(double), -2, C_loc);
 	...
 	CoCoMemcpy(C, C_buf,  M * N *sizeof(double), C_loc, -2);
    CoCoFree(A_in_C_loc, C_loc);
    CoCoFree(B_in_C_loc, C_loc);
