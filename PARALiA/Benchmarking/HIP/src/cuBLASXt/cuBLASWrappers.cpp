...
double cuBLASDdotWrap(long int N, double* x, long int incx, double* y, long int incy, double* result, double cpu_ratio, short dev_num, int dev_ids[] ){
  ...
  hipSetDevice(dev_ids[0]);
	...
	lprintf(lvl, "cuBLAS initialization -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == hipblasDdot(handle0, N, x, incx, y, incy, result));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
	...
	/// Free local buffers
	assert(HIPBLAS_STATUS_SUCCESS == hipblasDestroy(handle0));
	CoCoSyncCheckErr();
...
double cuBLASDaxpyWrap(long int N, double alpha, double* x, long int incx, double* y, long int incy, double cpu_ratio, short dev_num, int dev_ids[] ){
  ...
  hipSetDevice(dev_ids[0]);
	...
	lprintf(lvl, "cuBLAS initialization -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == hipblasDaxpy(handle0, N, &alpha, x, incx, y, incy));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
...
double cuBLASSaxpyWrap(long int N, float alpha, float* x, long int incx, float* y, long int incy, double cpu_ratio, short dev_num, int dev_ids[] ){
  ...
  hipSetDevice(dev_ids[0]);
	...
	lprintf(lvl, "cuBLAS initialization -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == hipblasSaxpy(handle0, N, &alpha, x, incx, y, incy));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
...
double cuBLASDgemvWrap(char TransA, long int M, long int N, double alpha, double *A, long int lda,
	...
	hipblasOperation_t gpu_op_A = OpCharToCublas(TransA);
	hipblasStatus_t stat;
	hipblasHandle_t handle0;
	...
	lprintf(lvl, "cuBLAS initialization -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == hipblasDgemv(handle0, gpu_op_A, M, N, &alpha, A, lda, x, incx, &beta, y, incy));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
