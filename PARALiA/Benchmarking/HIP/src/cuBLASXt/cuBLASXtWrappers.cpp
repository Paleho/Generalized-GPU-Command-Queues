...
double cuBLASXtDgemmWrap(char TransA, char TransB, long int M, long int N, long int K, double alpha, double* A, long int ldA, double* B, long int ldB, double beta, double* C, long int ldC, long int T, double cpu_ratio, short dev_num, int dev_ids[] ){
	...
	cublasOperation_t gpu_op_A = (cublasOperation_t) OpCharToCublas(TransA), gpu_op_B = (cublasOperation_t) OpCharToCublas(TransB);
	hipblasStatus_t stat;
	cublasXtHandle_t handle0;

	/// Required allocations for device
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtCreate(&handle0));
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtDeviceSelect(handle0, dev_num, dev_ids));
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtSetBlockDim(handle0, T));
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtSetCpuRoutine(handle0, CUBLASXT_GEMM, CUBLASXT_DOUBLE, (void*) &cblas_dgemm_wrap_for_cublasXt));
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtSetCpuRatio(handle0, CUBLASXT_GEMM, CUBLASXT_DOUBLE, cpu_ratio));
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtSetPinningMemMode(handle0, CUBLASXT_PINNING_ENABLED));
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
	lprintf(lvl, "cuBLASXt initialization/pinning -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtDgemm(handle0, gpu_op_A, gpu_op_B, M, N, K, &alpha, A, ldA, B, ldB, &beta, C, ldC));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
...
double cuBLASXtSgemmWrap(char TransA, char TransB, long int M, long int N, long int K, float alpha, float* A, long int ldA, float* B, long int ldB, float beta, float* C, long int ldC, long int T, double cpu_ratio, short dev_num, int dev_ids[] ){
	...
	lprintf(lvl, "cuBLASXt initialization/pinning -> t_init = %lf ms\n", cpu_timer*1000);
    	cpu_timer = csecond();
#endif
	assert(HIPBLAS_STATUS_SUCCESS == cublasXtSgemm(handle0, gpu_op_A, gpu_op_B, M, N, K, &alpha, A, ldA, B, ldB, &beta, C, ldC));
	CoCoSyncCheckErr();
#ifdef TEST
	cpu_timer = csecond() - cpu_timer;
	lprintf(lvl, "cuBLASXt execution time -> t_kernel = %lf ms\n", cpu_timer*1000);
