...
///
/// \brief A hipblasDaxpy micro-benchmark
///
...
int main(const int argc, const char *argv[]) {
	...
	if (dev_id != -1){
		...
		massert(hipSuccess == hipMemGetInfo(&free_cuda_mem, &max_cuda_mem),
			"daxpy_microbench_gpu: hipMemGetInfo failed");
	...
	for ( int itt = 0; itt <1; itt++){
		if(dev_id != -1){
			assert(HIPBLAS_STATUS_SUCCESS == hipblasDaxpy(handle0, maxDim, &alpha, x_dev, incx, y_dev, incy));
	...
	for (long int T = MIN_DIM_BLAS1; T < maxDim + 1; T+=step){
		if (T >= step * 16) step *=2;
		fprintf(stderr,"Running hipblasDaxpy-> T = %d:\n", T);
		...
		for (sample_sz = 1; sample_sz < MICRO_MAX_ITER + 1; sample_sz++) {
			cpu_timer = csecond();
			if(dev_id != -1){
				assert(HIPBLAS_STATUS_SUCCESS == hipblasDaxpy(handle0, T, &alpha, x_dev, incx, y_dev, incy));
				hipStreamSynchronize(host_stream);
			}
