...
///
/// \brief A hipblasDdot micro-benchmark
///
...
int main(const int argc, const char *argv[]) {
	...
	if (dev_id != -1){
		...
		massert(hipSuccess == hipMemGetInfo(&free_cuda_mem, &max_cuda_mem),
			"ddot_microbench_gpu: hipMemGetInfo failed");
	...
	for ( int itt = 0; itt <1; itt++){
		if(dev_id != -1){
			assert(HIPBLAS_STATUS_SUCCESS == hipblasDdot(handle0, maxDim, x_dev, incx, y_dev, incy, &result));
	...
	for (long int T = MIN_DIM_BLAS1; T < maxDim + 1; T+=step){
		if (T >= step * 16) step *=2;
		fprintf(stderr,"Running hipblasDdot-> T = %d:\n", T);
		...
		for (sample_sz = 1; sample_sz < MICRO_MAX_ITER + 1; sample_sz++) {
			cpu_timer = csecond();
			if(dev_id != -1){
				assert(HIPBLAS_STATUS_SUCCESS == hipblasDdot(handle0, T, x_dev, incx, y_dev, incy, &result));
				hipStreamSynchronize(host_stream);
			}
