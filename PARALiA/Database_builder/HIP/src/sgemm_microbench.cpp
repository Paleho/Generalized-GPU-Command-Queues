...
int main(const int argc, const char *argv[]) {
	...
	if (dev_id != -1){
		...
		massert(hipSuccess == hipMemGetInfo(&free_cuda_mem, &max_cuda_mem),
			"sgemm_microbench_gpu: hipMemGetInfo failed");
		if (free_cuda_mem < 1.0*PROBLEM_GPU_PERCENTAGE/100*max_cuda_mem)
			error("sgemm_microbench_gpu: Free memory is much less than max ( Free: %zu, Max: %zu ),\
			device under utilization\n", free_cuda_mem, max_cuda_mem);
	}

	char *filename = (char *) malloc(1024* sizeof(char));
	sprintf(filename, "%s/Benchmark-Results/cublasSgemm_dev-%d_TransA-%c_TransB-%c_%s.log",
	...
	long int maxDim = CoCoGetMaxDimSqAsset2D(3, sizeof(float), STEP_TRANS, dev_id);
	...
	float *A_buf, *B_buf, *C_buf;
  	A_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id);
  	B_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id);
  	C_buf = (float*) CoCoMalloc(maxDim * maxDim * sizeof(float), dev_id);
	CoCoSyncCheckErr();
	...
	for ( int itt = 0; itt <1; itt++){
		if(dev_id != -1){
			assert(HIPBLAS_STATUS_SUCCESS == hipblasSgemm(handle0, gpu_op_A, gpu_op_B,
				maxDim, maxDim, maxDim, &alpha, A_buf, ldA, B_buf, ldB, &beta, C_buf, ldC));
			hipStreamSynchronize(host_stream);
		}
		else cblas_sgemm(CblasColMajor, cpu_op_A, cpu_op_B,
			maxDim, maxDim, maxDim, alpha, A_buf, ldA, B_buf, ldB, beta, C_buf, ldC);
	}
	CoCoSyncCheckErr();
	...
	for (long int T = MIN_DIM_BLAS3; T < MAX_DIM_BLAS3; T+=step){
		...
		sprintf(powa_filename, "sgemm_microbench_powa.log");
		...
		CoCoSyncCheckErr();
#ifdef ENABLE_POWA
		if(dev_id != -1) nvem_data = NvemStopMeasure(dev_id, "Energy measure sgemm_microbench");
		else{
			nvem_data = (NvemStats_p) malloc(sizeof(struct nvem_results));
			strcpy(nvem_data->name, "Energy measure sgemm_microbench");
