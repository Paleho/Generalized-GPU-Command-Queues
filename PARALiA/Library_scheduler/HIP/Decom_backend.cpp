...
void* prepareAsync_backend(void* compressed_data){
	...
	if (prep_data->pin_internally) hipHostRegister(prep_data->adrs,prep_data->pin_bytes,hipHostRegisterPortable);
#ifdef DEBUG
...
void Decom2D::prepareAsync(pthread_t* thread_id, pthread_attr_t attr){
	...
	if (hipSuccess!=hipPointerGetAttributes(&attributes, prep_data->adrs)) pin_internally = 1;
...
void Decom2D::resetProperties(){
	if (pin_internally) hipHostUnregister(adrs);
}
